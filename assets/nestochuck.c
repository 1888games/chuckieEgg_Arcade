...
unsigned char buffer[64];
	...
	for (r = 0; r < 256; r++)
	{
		...
		for (s = 0; s < 8; s++) {
			...
			buffer[0 + s] = mmbuf[ptr2 + 8 + s];
			buffer[8 + s] = mmbuf[ptr2 + 0 + s];
		}
...
void getromsprites()
{
	...
	for (r = 0; r < 64; r++)
	{
		...
		for (s = 0; s < 8; s++) {
			...
			buffer[0 + s] = mmbuf[ptr2 + 8 + s];
			buffer[8 + s] = mmbuf[ptr2 + 16 + s];
			buffer[16 + s] = mmbuf[ptr2 + 24 + s];
			buffer[24 + s] = mmbuf[ptr2 + 0 + s];

			buffer[32 + s] = mmbuf[ptr2 + 40 + s];
			buffer[40 + s] = mmbuf[ptr2 + 48 + s];
			buffer[48 + s] = mmbuf[ptr2 + 56 + s];
			buffer[56 + s] = mmbuf[ptr2 + 32 + s];
		}
